...
const lead_performance_actor = assign({ 
  actors: (context, event) => {
  const name = event.to
...
const resume_actor = assign({ 
  actors: (context, event) => {
  const name = event.to
  return [ ...context.actors, { name, ref: spawn(attest_lead_performance.withContext({ name }), { name, sync: true }) } ]
} })

const trigger = (event_type) => { send(event_type, { to: (_, event) => event.to }) }
  ...
  states: {
    idle: {
      on: {
        ...
        user_PUBLISH_HOSTING_PLAN: { actions: plan_actor },
        ...
        _ORGANIZE_PERFORMANCE_CHECK: { actions: send('ORGANIZE_PERFORMANCE_CHECK', { to: (_, event) => event.to }) },
